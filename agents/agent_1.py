...
Generated by DEEPTHOUGHT by M Davis 6/5/2026
=============================================
Flow:
  START
    │
...
#  from config.state_data import STATE_REGULATORY_INFO  <-- appears to be unecessary (and not in state_data
...
def node_geocode(state: AgentState) -> AgentState:
    ...
    """
    logger.info(f"Node 1 - Geocoding: {state['address']}")

    try:
        # Call geocoding tool from tools/geocoding.py
        result = geocode_address(state["address"])

        if result["success"]:
            state["latitude"] = result["latitude"]
            state["longitude"] = result["longitude"]
            state["formatted_address"] = result.get(
                "formatted_address",
                state["address"]
            )
            state["geocode_confidence"] = result.get(
                "confidence", "medium"
            )
            logger.info(
                f"  Geocoded: {state['latitude']}, "
                f"{state['longitude']} "
                f"(confidence: {state['geocode_confidence']})"
            )
...
def node_find_utility(state: AgentState) -> AgentState:
    """
    ...
    try:
        ...
        if result["success"]:
            state["utility_name"] = result.get(
                "utility_name", "Unknown"
            )
            state["utility_id"] = result.get(
                "utility_id", "Unknown"
            )
            state["state"] = result.get(
                "state", "Unknown"
            )
            state["iso_region"] = result.get(
                "iso_region",
                "ISO New England (ISO-NE)"
            )
            logger.info(
                f"  Found utility: {state['utility_name']} "
                f"({state['state']})"
            )
            ...
            logger.error(
                f"  Utility not found: {state['error']}"
            )

    except Exception as e:
        state["error"] = f"Utility lookup exception: {str(e)}"
        state["error_node"] = "find_utility"
        state["error_suggestion"] = (
...
def node_get_regulatory_info(state: AgentState) -> AgentState:
    """
    ...
    try:
        ...
        if reg_info:
            state["regulatory_info"] = reg_info
            state["puc_name"] = reg_info.get(
                "puc_name", "Unknown"
            )
            state["puc_website"] = reg_info.get(
                "puc_website", "Unknown"
            )
            state["energy_office"] = reg_info.get(
                "energy_office", "Unknown"
            )
            logger.info(
                f"  Regulator: {state['puc_name']}"
            )
            ...
            state["puc_website"] = "dsireusa.org"
            state["energy_office"] = (
...
- Input Address: {state.get('address', 'Unknown')}
- Formatted Address: {state.get('formatted_address', 'Unknown')}
- Coordinates: {state.get('latitude')}, {state.get('longitude')}
- Geocode Confidence: {state.get('geocode_confidence', 'Unknown')}

UTILITY IDENTIFICATION:
- Electric Utility: {state.get('utility_name', 'Unknown')}
- Utility ID: {state.get('utility_id', 'Unknown')}
- State: {state.get('state', 'Unknown')} \
({state.get('state', '')})
- ISO/RTO Region: {state.get('iso_region', 'ISO New England')}

REGULATORY CONTACTS:
- Primary Regulator: {state.get('puc_name', 'Unknown')}
- Regulator Website: {state.get('puc_website', 'Unknown')}
- State Energy Office: {state.get('energy_office', 'Unknown')}

ADDITIONAL REGULATORY DETAILS:
{json.dumps(state.get('regulatory_info', {}), indent=2)}
...
=====================
Address:    {state.get('formatted_address', state.get('address'))}
Utility:    {state.get('utility_name', 'Unknown')}
State:      {state.get('state', 'Unknown')}
ISO Region: {state.get('iso_region', 'ISO New England')}
Regulator:  {state.get('puc_name', 'Unknown')}
Website:    {state.get('puc_website', 'Unknown')}
...
def node_handle_error(state: AgentState) -> AgentState:
    ...
    error_node = state.get("error_node", "Unknown step")
...
- Coordinates:  {state.get('latitude')}, {state.get('longitude')}
- Utility:      {state.get('utility_name', 'Not identified')}
- State:        {state.get('state', 'Not identified')}
...
def route_after_find_utility(state: AgentState) -> str:
    """
...
def run_agent1(address: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "utility_id": final_state.get("utility_id"),
